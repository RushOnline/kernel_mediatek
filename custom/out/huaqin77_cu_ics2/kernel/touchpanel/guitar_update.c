...
#define UPDATE_FILE_PATH_2   "/system/etc/firmware/gt813.bin"
#define UPDATE_FILE_PATH_1   "/sdcard/goodix/gt813.bin"
...
static int get_ic_fw_msg(struct i2c_client *);
...
extern int i2c_write_dummy(struct i2c_client *, u16);
extern int gt813_load_config(void);


#pragma pack(1)
typedef struct 
{
    u8  type;          /* module type info */
    u16 version;       /* FW version */
    u8  msk_ver[4];    /* MASK version */
    u8  st_addr[2];    /* flash start address */
    u16 lenth;         /* FW length */
    u8  chk_sum[3];
    u8  force_update[6];	/* force update flag, when eque with "goodix" force update */
}st_fw_head;
#pragma pack()
...
*
* @Returns: equal: ture, unequal: false
*/
...
static void guitar_reset(int reset_duration_time_ms)
{
...
* @Param: addr		i2c client registers addr
* @Param: msg		buf
 ...
 *
 */
static int get_ic_fw_msg(struct i2c_client *client)
{
    s32 ret = 0;
    s32 i = 0;
    u8 buf[32];
...
static void guitar_leave_update_mode(void)
{
    ...
    /* goodix's rubbish code forbin it !
...
u8 load_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* data, u8* path)
{
    ...
    if (path){
        ...
        /* Begin to search update file , search 15s */
        for (i = 0; i < SEARCH_FILE_TIMES; i++) {
            
			//update_msg.file = filp_open(UPDATE_FILE_PATH_1, O_RDWR, 0666);
			update_msg.file = filp_open(UPDATE_FILE_PATH_1, O_RDONLY, 0666);	/* O_RDONLY */
            if (IS_ERR(update_msg.file)) {
				
				//update_msg.file = filp_open(UPDATE_FILE_PATH_2, O_RDWR, 0666); /* O_RDWR */
				update_msg.file = filp_open(UPDATE_FILE_PATH_2, O_RDONLY, 0666); /* O_RDONLY */
                if (IS_ERR(update_msg.file)) {
                    DEBUG_UPDATE("%3d:Searching file...\n", i);
                    msleep(3000);
                    continue;
				} else {
                    break;	/* find update file at path2, break  */
                }

            } else {
                break;		/* find update file at path1, break  */
            }
        }
            ...
            if (fw_head->version > update_msg.ic_fw_msg.version 
               ...
               // file->f_pos = FW_HEAD_LENGTH + FILE_HEAD_LENGTH + IGNORE_LENGTH;
				...
				if (is_equal(fw_head->force_update, "GOODIX", 6)) {
                    update_msg.gt_loc 
					= update_msg.file->f_pos - FW_HEAD_LENGTH + sizeof(st_fw_head) - sizeof(fw_head->force_update);
                }
...
static int guitar_update_proc(void *v_cli)
{
    ...
    /* Reset guitar and send config file */
 ...
 *
 */
int init_update_proc(struct i2c_client *client)
{
	...
	/* get ic firmware info, if cann't read, force update */
